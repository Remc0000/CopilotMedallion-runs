# Run Spec 20260605-121537-7d254e

## Inputs
- Workspace: `4d720ee3-ed92-48f2-b755-c9b2e1da8541`
...
- Use defensive column references and validate column existence before every transformation.
- Use alias-prefixed joins for all multi-table joins and explicitly rename shared columns.
- Assert required columns before every groupBy/agg operation.
- For REST/API responses, use `if x is None: raise RuntimeError(...)` before any `.get(...)` access.
- Create schemas with `CREATE SCHEMA IF NOT EXISTS` and write only via `saveAsTable('<schema>.<table>')`.
- Never write target lakehouse tables using raw abfss `.save()` paths.
- Include parameter cells at the start of every notebook.
...
- Ensure every layer writes discoverable Delta tables into its schema.
- Every notebook code cell must begin with a short explanatory comment block using `# ---`.
...
Land each source table unchanged into the `bronze` schema with source fidelity preserved.
...
- salesorderheader: partition by year(orderdate) helper column.
- salesorderdetail: partition by salesorderid hash bucket helper.
- Remaining master tables: no partitioning due to small size.

Write pattern:
- Delta overwrite with schema evolution enabled.
- One table processed per loop iteration.
- Persist source row counts for reconciliation.
...
- Add _silver_ts and source audit metadata.
- Normalize timestamps to UTC where applicable.
- Drop binary column thumbnail_photo from product unless explicitly needed for analytics.
- OPTIMIZE all Silver tables after load.

Deduplication keys:
- silver.address: address_id
- silver.customer: customer_id
- silver.customer_address: (customer_id, address_id)
- silver.product: product_id
- silver.product_category: product_category_id
- silver.product_description: product_description_id
- silver.product_model: product_model_id
- silver.product_model_product_description: (product_model_id, product_description_id, culture)
- silver.sales_order_header: sales_order_id
- silver.sales_order_detail: sales_order_detail_id

Silver enrichment:
- customer:
  - Create salesperson_username from sales_person.
  - When value contains `\`, retain only username portion.
  - Create salesperson_clean for Gold dimension creation.
- product:
  - Derive is_discontinued from discontinued_date.
  - Derive is_currently_sellable using sell_start_date, sell_end_date, and discontinued_date.
- sales_order_header:
  - Derive order_year, order_month, order_date_key.
  - Derive ship_date_key when ship_date exists.
- address:
  - Standardize city and postal_code formatting.

Note:
- User requested ProductModel Name → modelname. The supplied ProductModel schema contains only ProductModelID, rowguid, and ModifiedDate. No Name column exists. Gold product dimension will therefore use ProductModelID and English description linkage only unless a richer ProductModel source is added.

## Gold

Target star schema focused on sales reporting, regional analysis, discount analysis, and trend reporting.
...
Implementation:
- Join customer to the billing address referenced by sales_order_header.bill_to_address_id.
- Retain only analytics-relevant attributes:
  - customer_id
  - company_name
  - email_address
  - city
...
Alternative note:
- Customer and Address have no direct key relationship. The requested design can only be achieved by using SalesOrderHeader as the linkage path between CustomerID and AddressID. CustomerAddress will not participate.
...
Transformation:
- Extract username from domain\username values.
- Remove domain prefix.
- Create one row per unique salesperson.
...
Move descriptive order attributes out of the fact:
- sales_order_id
- revision_number
- status
- ship_method
...
Implementation:
- Filter product_model_product_description to culture = 'en'.
- Join Product → ProductModelProductDescription → ProductDescription.
...
Regional reporting:
- Region is represented by customer city/postal geography available from Address.
- Map visuals will use city and postal code geography fields.
...
- Maximum Order Value = MAXX(VALUES(dim_order[sales_order_id]), CALCULATE(SUM(fact_sales_order[net_sales_amount])))

Discount:
- Total Discount Amount = SUM(fact_sales_order[line_discount_amount])
- Discount % = DIVIDE([Total Discount Amount],[Gross Sales])
- Average Discount % = AVERAGEX(fact_sales_order,fact_sales_order[unit_price_discount])
- Maximum Discount % = MAX(fact_sales_order[unit_price_discount])

Operations:
- Average Freight = AVERAGE(fact_sales_order[freight])
- Maximum Freight = MAX(fact_sales_order[freight])

## Report

### Page 1 — Executive Sales Overview
Visuals:
- KPI cards: Total Sales, Average Sales, Maximum Sales, Total Orders
- Monthly sales trend line chart using OrderDate hierarchy
- Sales by Product Category clustered column chart
- Discount % KPI
- Slicers: Date, Category, SalesPerson

### Page 2 — Regional Performance
Visuals:
- Filled map or bubble map using city and postal code geography
- Top 10 cities by Total Sales
- Bottom 10 cities by Total Sales
- Average Sales by City
- Maximum Sales by City
- Regional sales trend line
...
- Largest discount recipients table
- Salespeople with highest Average Discount %
- Salespeople with highest Maximum Discount %

### Page 4 — Product Performance
Visuals:
- Category/Subcategory drill-down chart
- Product sales ranking
- Product margin proxy using Gross Sales vs Standard Cost
...
Role:
- AI Sales Performance Analyst for the SalesLT sales reporting environment.

Grounding:
- Use only the Direct Lake semantic model.
- Prioritize measures and governed dimensions.
- Answer using current report definitions and business calculations.
...
- Which cities are underperforming compared to the prior month?
- What is the average and maximum sales value by city?
...
- What products are frequently associated with high discounts?

Guardrails:
- Do not invent data not present in the semantic model.
- Always use published measures before creating ad hoc calculations.
- Distinguish clearly between Gross Sales, Net Sales, and Discount %.
- Explain filters and time periods used in answers.
- Prefer aggregated insights over row-level sensitive detail.
- If requested information is unavailable because the source schema lacks a field, explicitly state the limitation.
- Do not infer geography beyond available city and postal code data.
- Remain focused on sales analytics and operational reporting.
