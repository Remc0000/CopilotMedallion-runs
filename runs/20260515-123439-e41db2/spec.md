...
- **Dim_Product** — join product, productcategory, productdescription, productmodel, productmodelproductdescription. Please notice that in productcategory there is a self join: in the end I have the category (which is the parent category) and a subcategory. For this self join, you MUST alias the two instances of `ProductCategory` as separate relations (for example, `pc_child` and `pc_parent`) and reference all join keys with the alias prefix. Use the child category row from `Product.ProductCategoryID = pc_child.ProductCategoryID`, then left join the parent category row on `pc_child.ParentProductCategoryID = pc_parent.ProductCategoryID`. Do not reference unqualified `parent_product_category_id` / `ParentProductCategoryID` after the self join, because this causes ambiguous-column failures. Materialize explicit output columns for both levels, e.g. `subcategory_id`, `subcategory_name`, `category_id`, and `category_name`, where subcategory comes from the child alias and category comes from the parent alias. If a product is already at the top category level and has no parent, keep the child values and allow the parent/category fields to be null.
...
- For `Dim_Product`, `ProductCategory` is a self-join and must use distinct aliases with fully qualified column references for `ProductCategoryID` and `ParentProductCategoryID`; do not use unqualified category columns anywhere in the join chain.