...
- Product enrichment preparation:
  - Create product_status flags from sell_start_date, sell_end_date, discontinued_date.
- Address standardization:
  - Normalize city, state_province, country_region.
...
- Build parent-child category flattening:
  - category_name from parent category.
  - subcategory_name from child category.
- Include:
  - product_id
  - product_name
  - product_number
...
- Measures stored:
  ...
  - tax_amount (allocated from header where required)
  - freight_amount (allocated from header where required)
- Fact excludes descriptive attributes already moved into dimensions.
...
- Every product category assignment resolves to either category or subcategory.
...
- Sales by Category and Subcategory treemap.
- Top and bottom performing product categories.
- Dynamic category slicers.

Page 2: Regional Performance
- Filled map and bubble map using:
  - CountryRegion
  - StateProvince
  - City
...
- Largest discount contributors by salesperson.
- Scatter plot:
  - Sales vs Discount %.
- Product category discount analysis.

Page 4: Product Performance
- Category → Subcategory → Product hierarchy drilldown.
- Sales by category.
