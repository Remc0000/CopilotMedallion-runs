...
### Iteration 3 — 2026-06-05 09:28:23Z — failed layer: reporting (run: 20260605-091205-f71fda)
- **Root cause (1-line summary)**: Reporting stage again terminated with a generic session-cancelled error; semantic-model/report generation likely attempted to bind visuals or relationships to columns/measures that were absent or named differently than expected.
- **Cross-table audit**:
  - Address: no — only consumed through published gold dimensions.
  - Customer: no — only consumed through published gold dimensions.
  - CustomerAddress: no — not used by reporting artifacts.
  - Product: no — only consumed through published gold dimensions.
  - ProductCategory: no — only consumed through published gold dimensions.
  - ProductDescription: no — only consumed through published gold dimensions.
  - ProductModel: no — only consumed through published gold dimensions.
  - ProductModelProductDescription: no — only consumed through published gold dimensions.
  - SalesOrderDetail: no — only consumed through fact_sales_order.
  - SalesOrderHeader: no — only consumed through dimensions and fact tables.
- **Fix approach**: GENERALIZE — the failure pattern is reporting-contract validation across all semantic-model and report objects rather than a single source table issue.
- **What was changed**:
  - Added mandatory semantic-model column contracts for every published gold table.
  - Required validation of measures, hierarchies, and relationships before report creation.
  - Restricted reports to a defined set of exposed semantic-model objects and prohibited inferred field references.
...
- Validate the following minimum column contracts before publishing:
  - dim_order_date: date_key
  - dim_ship_date: ship_date_key
  ...
  - fact_sales_order: order_date_key, ship_date_key, customer_key, salesperson_key, order_key, product_key
- Do not create measures, hierarchies, perspectives, or relationships that reference columns outside these validated contracts.
- If any required table or column is missing, stop semantic-model publication and emit the exact missing object name.
...
- Before creating any visual, validate the referenced table, column, hierarchy, and measure individually.
- Do not rely on autogenerated field discovery or inferred measure names.
- Only use objects successfully published in the semantic model metadata returned by the semantic-model creation step.
- If zero validated visuals can be created from the published semantic model, stop report generation with an explicit validation error instead of executing additional Spark statements.
